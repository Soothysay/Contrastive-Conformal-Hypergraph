...
parser.add_argument('--dataset', type=str, default='walmart-trips', choices = ['house-bills', 'house-committees', 'walmart-trips','cora','dblp', 'citeseer', 'pubmed']) #'house-bills'
...
parser.add_argument('--temperature', type=float, default=0.3) #0.2 and 1000
...
parser.add_argument('--coverage_warmup_epochs', type=int, default=3000)
parser.add_argument('--contrastive_loss_weight', type=float, default=1.0)
parser.add_argument('--late_contrastive_loss_weight', type=float, default=0.001)
parser.add_argument('--non_conftr_size_loss_weight', type=float, default=2.0)
...
def dataset_preprocess(dataset_name,types=None):
    if dataset_name in ['cora','dblp'] and types=='CA': # 'Cora_CC', 'CiteSeer_CC', 'PubMed_CC'
        ...
        num_nodes = len(node_id_map)
        hypergraph=[v for k, v in hypergraph.items()]
    elif dataset_name in ['house-committees','house-bills','walmart-trips'] and types==None:
    ...
    if dataset_name not in ['house-committees','house-bills','walmart-trips'] and types==None:
        ...
        if (os.path.exists(model_checkpoint)) and (not args.retrain):
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                if args.conftr:
                    ...
                    loss = pred_loss + args.non_conftr_size_loss_weight * size_loss + degree_loss
                loss.backward()
                #print(epoch)
                optimizer.step()
                if args.verbose:
                    if epoch % args.log_interval == 0:
                        print('Epoch:', epoch, 'Loss:', loss.item(), 'Size Loss:', size_loss.item(), 'Pred Loss:', pred_loss.item(), 'Cont Loss:', cont_loss.item())
