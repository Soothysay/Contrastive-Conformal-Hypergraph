...
parser.add_argument('--dataset', type=str, default='congress-bills', choices = ['house-bills', 'house-committees', 'walmart-trips','cora','dblp', 'citeseer', 'pubmed','congress-bills']) #'house-bills'
...
parser.add_argument('--size_loss_weight', type=float, default=2)
...
parser.add_argument('--temperature', type=float, default=0.25)#0.2
...
parser.add_argument('--edge_topk', type=int, default=1000)
...
def main(args):
    ...
    # Aggregate metrics over all completed runs.
    avg_tau2res = {}
    for key in tau2res[run].keys():
        if key not in avg_tau2res:
            avg_tau2res[key] = {}
        for subkey in tau2res[run][key].keys():
            if subkey not in avg_tau2res[key]:
                avg_tau2res[key][subkey] = []
            avg_tau2res[key][subkey].append(tau2res[run][key][subkey])
    for key in avg_tau2res.keys():
        for subkey in avg_tau2res[key].keys():
            avg_tau2res[key][subkey] = np.mean(avg_tau2res[key][subkey])
    print('Average results over all runs:', avg_tau2res,flush=True)
main(args)
